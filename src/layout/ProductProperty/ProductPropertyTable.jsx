...
const ProductPropertyTable = props => {
    ...
    const mapState = useCallback(state => ({ ...state.productProperty }), [])
    ...
    const productColumns = useCallback([
        ...
        {
            title: 'Тип',
            render: row => row.type && row.type.title,
        },
        {
            title: 'Описание',
            render: row => row.description && (row.description.length > 30 ? row.description.slice(0, 30) + '...' : row.description),
    ...
    return (
        <Paper>
            <Label><Text variant="h6">Свойства продукта</Text></Label>
            ...
            <ButtonsWrapper>
                ...
                />
                <ButtonNew>
                    <Button onClick={handleCreateClick}>Новое свойство</Button>
                </ButtonNew>
            </ButtonsWrapper>
...
export default ProductPropertyTable