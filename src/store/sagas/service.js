...
import {
    actionCreator,
    constants,
    SERVICE_TOGGLE_ACTIVE_REQUEST,
    SERVICE_TOGGLE_ACTIVE_SUCCESS,
    SERVICE_TOGGLE_ACTIVE_FAILURE,
...
export default function* watchFetch() {
    ...
    yield takeLatest([SERVICE_TOGGLE_ACTIVE_REQUEST], toggleItemAsync);
}
...
function* toggleItemAsync({ payload }) {
    try {
        ...
        yield put({ type: SERVICE_TOGGLE_ACTIVE_SUCCESS })
        const message = withMessage.updateSuccess()
        yield put(pushMessage(message))

    } catch (error) {
        yield put({ type: SERVICE_TOGGLE_ACTIVE_FAILURE })
        yield put(pushMessage(errorHandler(error)))
    }
}